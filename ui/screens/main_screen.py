...
def build_main_screen(parent: ttk.Frame, *, presenter, controller, ui) -> None:
    ...
    ttk.Label(st, text='????').grid(row=6, column=0, padx=10, pady=(2, 2), sticky='w')
    ttk.Label(st, textvariable=presenter.auto_state_var).grid(row=6, column=1, padx=10, pady=(2, 2), sticky='w')
    ttk.Label(st, text='????').grid(row=7, column=0, padx=10, pady=(2, 2), sticky='w')
    ...
    _kv(od_box, 4, '???????', ttk.Label(od_box, textvariable=presenter.max_od_pp_rob_var))
    _kv(od_box, 5, '????????', ttk.Label(od_box, textvariable=presenter.max_od_fit_res_var))
    _kv(od_box, 6, '??????', ttk.Label(od_box, textvariable=presenter.od_tilt_var))
    ...
    _kv(id_box, 4, '??????', ttk.Label(id_box, textvariable=presenter.id_slope_var))
    _kv(id_box, 5, '????(????)', ttk.Label(id_box, textvariable=presenter.id_endoff_var))
    _kv(id_box, 6, '???????', ttk.Label(id_box, textvariable=presenter.max_id_round_var), pady=(2, 8))

    _kv(all_box, 0, '?????', ttk.Label(all_box, textvariable=presenter.axis_dist_var), pady=(8, 2))
    _kv(all_box, 1, '?????max', ttk.Label(all_box, textvariable=presenter.conc_max_var))
    _kv(all_box, 2, '??????', ttk.Label(all_box, textvariable=presenter.axis_span_max_var))
    _kv(all_box, 3, '?????', ttk.Label(all_box, textvariable=presenter.len_meas_var), pady=(2, 8))

    ctrl = ttk.LabelFrame(top, text='??')
    ctrl.grid(row=0, column=2, sticky='ns')
    ttk.Button(ctrl, text='????', width=16, command=controller.start_measurement).pack(padx=10, pady=(10, 6))
    ttk.Button(ctrl, text='??', width=16, command=controller.stop_measurement).pack(padx=10, pady=6)
    ttk.Button(ctrl, text='????', width=16, command=controller.clear_measurement_results).pack(padx=10, pady=(6, 10))
    ...
    def _draw_group_header() -> None:
        ...
        groups = [
            ('??', ('idx', 'x_ui')),
            ('??', ('od_dev', 'od_round', 'od_e', 'od_phi_deg', 'od_ecc')),
            ('??', ('id_dev', 'id_round', 'id_e', 'id_phi_deg', 'id_ecc')),
            ('??', ('concentricity',)),
        ]
