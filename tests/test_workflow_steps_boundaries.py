...
def test_measure_row_build_inputs_is_covered_by_boundary_scan() -> None:
    assert _STEPS_DIR.joinpath("measure_row_build_inputs.py") in _step_files()


def test_measure_row_computation_result_is_covered_by_boundary_scan() -> None:
    assert _STEPS_DIR.joinpath("measure_row_computation_result.py") in _step_files()
