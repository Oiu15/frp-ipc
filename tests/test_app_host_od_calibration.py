...
class AppHostOdCalibrationTest(unittest.TestCase):
    def test_one_rev_progress_unwraps_across_zero_and_detects_done(self) -> None:
        host = _FakeOdCalHost()

        self.assertEqual(host._odcal_update_rev_progress(350.0), 0.0)
        self.assertEqual(host._odcal_update_rev_progress(10.0), 20.0)
        self.assertEqual(host._odcal_update_rev_progress(100.0), 110.0)
        self.assertFalse(host._odcal_rev_done())

        host._odcal_rev_progress_deg = 359.1
        self.assertTrue(host._odcal_rev_done())

    def test_degree_source_prefers_relative_angle_then_absolute_angle(self) -> None:
        host = _FakeOdCalHost()

        self.assertEqual(host._odcal_deg_from_point({"theta_rel": 361.9, "theta": 10}), 1)
        self.assertEqual(host._odcal_deg_from_point({"theta": -1.0}), 359)
        self.assertIsNone(host._odcal_deg_from_point({}))
    ...
    def test_mask_ranges_round_trip_wraparound_segments(self) -> None:
        ...
        self.assertEqual([idx for idx, value in enumerate(mask) if value], [0, 1, 2, 90, 91, 358, 359])
        self.assertEqual(host._odcal_mask_to_ranges(mask), [(358, 2), (90, 91)])
        self.assertIn("90~91", host._odcal_ranges_str([(90, 91)]))
    ...
    def test_detect_defect_mask_closes_gap_and_applies_padding(self) -> None:
        ...
        self.assertEqual([idx for idx, value in enumerate(mask) if value], [9, 10, 11, 12, 13])
        self.assertTrue(debug["segments"])
    ...
    def test_best_shift_helpers_align_template_to_current_defect(self) -> None:
        ...
        self.assertEqual(shift, 15)
        assert shift is not None
        self.assertEqual(debug["n"], 2)
        shifted = host._odcal_shift_mask(template, int(shift))
        self.assertEqual([idx for idx, value in enumerate(shifted) if value], [25, 26])
        self.assertEqual(host._odcal_best_shift_by_overlap(template, shifted)[0], 15)
    ...
    def test_prepare_sums_and_update_stats_apply_mapping_and_drop_rate(self) -> None:
        ...
        self.assertEqual(sums, [3.0, 5.0, 7.0])
        self.assertEqual(meta["defect_mode"], "OFF")
        self.assertEqual(host.odcal_sum_mean_var.get(), "5.00000")
        self.assertEqual(host.odcal_sum_min_var.get(), "3.00000")
        self.assertEqual(host.odcal_sum_max_var.get(), "7.00000")
        self.assertEqual(host.odcal_drop_rate_var.get(), "33.3%")
    ...
    def test_load_and_save_active_calibration_sync_ui_fields_and_template(self) -> None:
        ...
        self.assertEqual(host.odcal_B_active_var.get(), "1.23457")
        self.assertEqual(host.odcal_dref_var.get(), "180.250")
        self.assertEqual(host.odcal_cmd_var.get(), "M1,1")
        self.assertEqual(host.odcal_map_out1_var.get(), "R")
        self.assertEqual(host.odcal_angle_src_var.get(), "无角度")
        self.assertEqual(host.odcal_filter_var.get(), "中值(3)")
        self.assertEqual(host.odcal_outlier_sigma_var.get(), "2.5")
        self.assertEqual(host.odcal_defect_mode_var.get(), "TEMPLATE")
        self.assertEqual(host._odcal_defect_template_mask, mask)
        self.assertEqual(repo.saved, [{"B_active": 2.0}])
