...
def test_build_measure_row_from_sampling_characterizes_od_id_path() -> None:
    ...
    assert row.od_avg == 2.0
    assert row.od_dev == 0.0
    assert row.id_avg == 2.0
    assert row.id_dev == 0.0
    assert row.concentricity == 5.0
    assert row.concentricity == concentricity_list[-1]
    assert row.split_shift_deg == 1.25
    assert row.coax_unreliable is False
    assert row.id_mode == "dual"
    assert row.od_round_fit_mm == 0.3
    assert row.od_round_fit_rob_mm == 0.4
    assert row.id_round_fit_mm == 0.5
    assert row.id_round_fit_rob_mm == 0.6
    assert row.raw == "OD:od-raw  ID:id-raw"
...
def test_build_measure_row_from_sampling_characterizes_missing_id_single_path() -> None:
    legacy = _FakeLegacyFlow()
    ...
    assert row.idx == 3
    assert row.x_ui == 8.5
    assert row.x_abs == 124.0
    assert row.od_avg == 2.0
    assert row.od_dev == 0.0
    assert row.id_mode == "single"
    assert row.id_avg is None
    assert row.id_dev is None
    assert row.id_round is None
    assert row.id_runout is None
...
def test_build_measure_row_from_sampling_defaults_round_fit_failures() -> None:
    class LegacyWithFailingRoundFit(_FakeLegacyFlow):
        def od_round_fit_from_raw_points(self, *_args: Any, **_kwargs: Any) -> tuple[float, float]:
            raise RuntimeError("od round fit failed")

        def id_round_fit_from_raw_points(self, *_args: Any, **_kwargs: Any) -> tuple[float, float]:
            raise RuntimeError("id round fit failed")

    legacy = LegacyWithFailingRoundFit()
    recipe = _FakeRecipe()
    ...
    assert row.od_round_fit_rob_mm is None
    assert row.id_round_fit_mm is None
    assert row.id_round_fit_rob_mm is None
